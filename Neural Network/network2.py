...
class NN2:
    ...
    def train(self, x, y):
        sample_no = x.shape[0]
        dimen = x.shape[1]
        idxn = np.arange(sample_no)
        for t in range(self.epoch):
            np.random.shuffle(idxn)
            x = x[idxn,:]
            y = y[idxn]
            for i in range(sample_no):
                self.forward_backward(x[i,:].reshape([self.data_input, 1]), y[i,:].reshape([self.data_output, 1]))
    ...
    def fit(self, x):
        ...
        l = []
        for i in range(sample_no):
            self.prop_forward(x[i,:].reshape(self.data_input))
            y = self.nodes[-1]
